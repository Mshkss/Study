...
#pragma omp parallel num_threads(k)  // a, d
    {
        int thread_id = omp_get_thread_num();
        int chunk_size = Nmin / k;

        // Определяем диапазон для каждого потока
        int start = thread_id * chunk_size;
        int end = (thread_id + 1) * chunk_size;

        // Обрабатываем остаток последним потоком
        if (thread_id == k - 1) {
            end = Nmin;
        }

        // Выполняем операции в диапазоне, определенном для текущего потока
...
#pragma omp parallel num_threads(k) // c
    {
...
#pragma omp parallel num_threads(k)           // d
    {
        int thread_id = omp_get_thread_num();
        int chunk_size = Nmin / k;
...
int main() {
    ...
    const int Nmin = 5000000;
    const int Nmax = 3 * Nmin;
    const int k = 1; // количество потоков
